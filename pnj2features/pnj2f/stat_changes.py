...
def changesh5_jra():
    f1 = h5py.File('./results/class/jra/jra/PNJ010.h5', 'r')
    sample1 = f1['jra']['ua_PNJ'][:]
    sample2 = f1['jra']['uastd_PNJ'][:]

    ttest, pval = scipy.stats.ttest_ind(sample1, sample2)

    if pval > 0.05:
        print('We accept null hypothesis for JRA-55', 'p-value', pval)
    else:
        print('We REJECT null hypothesis for JRA-55', 'p-value', pval)
...
def changesh5():
    ...
    for i in np.arange(0, len(model_vec)):
        ...
        print('./results/class/' + period_vec1 + '/' + model_vec[i] + '/PNJ010.h5', 'r')

        f1 = h5py.File('./results/class/' + period_vec1 + '/' + model_vec[i] + '/PNJ010.h5', 'r')
        f2 = h5py.File('./results/class/' + period_vec2 + '/' + model_vec[i] + '/PNJ010.h5', 'r')
