    ...
    for opt in [0]:
        ...
        for pnjtype in pnjtypes:
            ...
            for i in np.arange(0, len(model_vec)):
                ...
                total = 0
                grp0 = f[model_vec[i]]
                if 'Day_normalPNJ' in grp0.keys():
                    total += len(f[model_vec[i]]['Day' + '_' + 'normalPNJ'])
                if 'Day_type2event' in grp0.keys():
                    total += len(f[model_vec[i]]['Day' + '_' + 'type2event'])
                if 'Day_type1event' in grp0.keys():
                    total += len(f[model_vec[i]]['Day' + '_' + 'type1event'])

                if 'Day_' + pnjtype in grp0.keys():
