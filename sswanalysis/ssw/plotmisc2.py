...
def plot_months(classify_model, classify_period, classify_level, pathh5, pathclass, pathplot):
    ...
    ''' Read hdf5 file'''
    f = h5py.File(pathclass + classify_period + '/' + classify_model + '/PNJ' + classify_level + '.h5', 'r')
    ...
    if 'uamap_normalPNJ' in grp0.keys():
        ...
        Day_normalPNJ = grp0['Day_normalPNJ'][:]
    ...
    if 'uamap_type1event' in grp0.keys():
        ...
        Day_type1event = grp0['Day_type1event'][:]
    ...
    if 'uamap_type2event' in grp0.keys():
        ...
        Day_type2event = grp0['Day_type2event'][:]
...
def plot_onewinter(y_final, ua_data, zg_data, type_PNJ, nc_lat_data, nc_lon_data, label):
    plt.figure()
    fig, axs = plt.subplots(10, 10, figsize=(10, 5))
    axs = axs.flatten()
    aux = 0
    for img, ax in zip(y_final, axs):
        if type_PNJ[aux] == 2:
            ax.imshow(img, origin='lower', cmap='autumn')
        elif type_PNJ[aux] == 1:
            ax.imshow(img, origin='lower', cmap='spring')
        else:
            ax.imshow(img, origin='lower', cmap='summer')
        aux += 1
    [axi.set_axis_off() for axi in axs.ravel()]
    plt.savefig('./plots/jra/jra/' + label + '_eventos.png', bbox_inches='tight')
    plt.close()
    gridx, gridy = np.meshgrid(nc_lon_data, nc_lat_data)

    plt.figure()
    fig, axs = plt.subplots(10, 10, figsize=(10, 5))
    axs = axs.flatten()
    for img, ax in zip(ua_data, axs):
        im = ax.contourf(gridx, gridy, img, cmap='coolwarm',
                         levels=np.linspace(-50, 110, 11),
                         vmin=-50, vmax=110,
                         extend='both')

    [axi.set_axis_off() for axi in axs.ravel()]
    cbar = fig.colorbar(im, ax=axs.ravel().tolist())
    cbar.set_label('ua [m/s]', rotation=90)

    plt.savefig('./plots/jra/jra/' + label + '_ua.png', bbox_inches='tight')
    plt.close()

    plt.figure()
    fig, axs = plt.subplots(10, 10, figsize=(10, 5))
    axs = axs.flatten()
    for img, ax in zip(zg_data, axs):
        im = ax.contourf(gridx, gridy, img, cmap='coolwarm',
    ...
    [axi.set_axis_off() for axi in axs.ravel()]
    cbar = fig.colorbar(im, ax=axs.ravel().tolist())
    cbar.set_label('zg[m]', rotation=90)

    plt.savefig('./plots/jra/jra/' + label + '_zg.png', bbox_inches='tight')
    plt.close()
